...
function writeFive(data){
    ...
    if(dayNightSwitch.dataset.switch === "Day"){
        for (let i=0; i<data.list.length/8; i++){
            ...
            fiveWind[i].innerHTML = `<b>Humidity</b> ${data.list[dayIndex[i]].main.humidity} %`
        ...
        for (let i=0; i<data.list.length/8; i++){
            ...
            fiveWind[i].innerHTML = `<b>Humidity</b> ${data.list[nightIndex[i]].main.humidity} %`
...
//Handles our "X" buttons allowing us to remove items from history
function removeData(e){
    ...
    //Save new array to local storage, remove the city's key, and get rid of div
    localStorage.setItem("cities", JSON.stringify(cities))
    div.innerHTML = ""
}

//Writes history buttons to the screen. 
